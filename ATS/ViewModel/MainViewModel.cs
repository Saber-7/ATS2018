...
namespace ATS
{
  ...
  //          YFormatter = (double x) => StaNamesDict[x];
        ...
        public void PathFind(string src, string tar)
        {
            Device now = new Device() { Name = src };
            Res = new List<OptionalRoutes>();
            SearchPathBy(now, Res, new OptionalRoutes(), src, tar, 0, new HashSet<int>());
            Res.Sort();
        }
        ...
        void SearchPathBy(Device Now, List<OptionalRoutes> res, OptionalRoutes tempRoutes, string src, string tar, int k, HashSet<int> used)
        {
            if (k > 20 || res.Count > 5)
                return;
            if (Now.Name == tar)
            ...
            {
                List<ATSRoute> PossibleRoutes = RC.Routes.FindAll((ATSRoute route) =>
                {
                ...
                if (PossibleRoutes != null)
                {
                    foreach (ATSRoute route in PossibleRoutes)
                    {
                        ...
                        used.Add(route.ID);
                        SearchPathBy(route.InSections.Last(), res, tempRoutes, src, tar, k + 1, used);
